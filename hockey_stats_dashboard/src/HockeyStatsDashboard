...
import { Upload, BarChart3, Users, X } from 'lucide-react'
...
export default function HockeyStatsDashboard() {
  ...
  const gameTypes = useMemo(() => {
    const unique = [...new Set(data.map(row => row['Game Type']).filter(s => s))].sort()
    return unique
  }, [data])
  ...
  const calculateChanceStats = (filteredData) => {
    ...
    // Total Chance Plays = Shots + Miss + Post (NOT Passes) - GAME ANALYTICS ONLY
    stats['Total Chance Plays'] = stats['Total Shots Chances'] + stats['Total Miss + Post']

    return stats
  }

  // Game Analytics with Team Split
  ...
  // Player stats for selected games
  const gamePlayerStats = useMemo(() => {
    ...
    // Add Total Chance Plays = Shots + Miss/Post
  ...
  // Player Analytics with Games Played count - using Number + Team as unique identifier
  // Total Chances = Shots + Passes + Miss/Post (DIFFERENT FROM GAME ANALYTICS)
  const playerAnalytics = useMemo(() => {
    ...
    if (selectedGameTypes.length > 0) {
      filtered = filtered.filter(row => selectedGameTypes.includes(row['Game Type']))
    }
    ...
    filtered.forEach(row => {
      ...
      // Use Number + Team as unique identifier
      ...
      // Track unique games for this player on this team
    ...
    // Convert Sets to numbers and add Total Chance Plays = Shots + Passes + Miss/Post (PLAYER ANALYTICS ONLY)
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {data.length === 0 ? (
          ...
          <div>
            ...
            {/* Player Table with Games Played */}
